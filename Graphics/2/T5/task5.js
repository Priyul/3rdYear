...
function createSphereMesh(longitudeBands, latitudeBands) {
    ...
    for (let latNumber = 0; latNumber <= latitudeBands; latNumber++) {
      ...
      for (let longNumber = 0; longNumber <= longitudeBands; longNumber++) {
        ...
        sphereVertices.push(1.0, 1.0, 1.0, 1.0); // Add a placeholder color
      }
    }
...
const lowResSphere = createSphereMesh(10, 10);
const highResSphere = createSphereMesh(50, 50);
let currentSphere = lowResSphere;


function drawPart(gl, vertices, indices, color) {
    const positionLocation = gl.getAttribLocation(program, 'a_position');
  ...
  document.getElementById('toggleMesh').addEventListener('click', () => {
    currentSphere = currentSphere === lowResSphere ? highResSphere : lowResSphere;
  });
